...
`meta/services.yml` declares a `tor:` block with `exclusive: false` and `primary: false`, an explicit override of the `svc-net-tor` provider's own default (`exclusive: true`, which most consuming roles inherit unmodified). Without `exclusive: false`, the moment an operator deploys `svc-net-tor` anywhere on the platform, Wazuh's clearnet vhost (`wazuh.{{ DOMAIN_PRIMARY }}`) would be silently **replaced** by its onion address rather than staying reachable alongside it (see `utils/cache/domains.py`'s `_inject_onion_domains`, and the passing unit test `tests/unit/python/utils/cache/test_domains.py::TestOnionDomainInjection::test_exclusive_primary_default_from_provider`, which asserts exactly that default). Wazuh is a security-monitoring/incident-response tool: losing guaranteed clearnet access to the dashboard as a side effect of an unrelated Tor deployment elsewhere is an operational risk specific to this kind of app, unlike a general-purpose SSO app where onion-only is an acceptable trade-off. `exclusive: false` keeps Wazuh dual-stack (clearnet **and** onion, when Tor is deployed); `primary: false` keeps the clearnet domain first in `domains['web-app-wazuh']`, so it stays the default/primary domain rather than the onion address.

**Both domains are now registered with the front proxy, and both drive working OIDC login.** `tasks/00_core.yml` includes `sys-stk-front-proxy` twice: once implicitly for the clearnet domain (unchanged), and once explicitly for `vars/main.yml`'s `WAZUH_DASHBOARD_ONION_DOMAIN` — the onion sibling `_inject_onion_domains` appends to `domains['web-app-wazuh']` — guarded by `when: WAZUH_DASHBOARD_ONION_DOMAIN | length > 0` so it is a no-op when `svc-net-tor` isn't deployed on this node. This relies on two supporting fixes:

- **`sys-front-tls`'s TLS-state resolution is domain-keyed, not app-id-keyed.** `roles/sys-front-tls/vars/main.yml`, `tasks/main.yml`, and `tasks/01_http.yml` previously resolved TLS enablement/mode from `lookup('tls', application_id, ...)` alone — a single value for the whole app, regardless of which domain a given `sys-stk-front-proxy` pass was actually registering. A second, onion-domain pass would have inherited the clearnet domain's `enabled: true` / `mode: letsencrypt`, producing a `listen 443 ssl` vhost Tor's plaintext SOCKS circuit can't use. Those three lookups now resolve `domain | default(application_id)` instead — domain-keyed when the caller supplies one (as `sys-stk-front-proxy` always does), falling back to the previous app-id-keyed behavior for any caller that doesn't. This is a platform-level fix in a shared role consumed by all 100+ apps on the platform; see `roles/sys-front-tls/vars/main.yml` for the exact change.
- **The dashboard's OIDC redirect URL is derived per request, not fixed at config-render time.** `templates/opensearch_dashboards.yml.j2` no longer sets a static `opensearch_security.openid.base_redirect_url`; `opensearch_security.openid.trust_dynamic_headers: true` makes OpenSearch Dashboards build the redirect URL from the incoming request's `X-Forwarded-Host`/`X-Forwarded-Proto` headers instead, so a login on either domain redirects back to that same domain rather than a single fixed one. This role's own `templates/proxy.conf.j2` (a fork of `sys-svc-proxy/templates/vhost/basic.conf.j2`) is what forwards `X-Forwarded-Host`; the shared `html.conf.j2` location template doesn't. Keycloak already accepts callbacks on either domain via the existing `redirect_uris.py` lookup, independent of this change.

**Verification status:** confirmed via `make lint-ansible`, `make test-lint`, and `make test-unit-python` only. This has not yet been exercised against a real `svc-net-tor` deployment — onion vhost registration, actual TLS behavior on the onion vhost, and an end-to-end OIDC login over the onion domain are all unverified against a live deploy. Treat this as implemented but not yet confirmed working; live confirmation is still needed before this should be considered fully resolved.
